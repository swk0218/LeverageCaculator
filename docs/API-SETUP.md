...
현재 18개 운영 상품은 증권상품시세 API의 상품 종가와 주식시세 API의 삼성전자(005930)·
SK하이닉스(000660) 종가를 함께 사용해 `full` 분석을 제공한다. 현물형 10종은 본주를 직접
분석 기준으로 사용하고, 선물형 6종과 TR형 ETN 2종은 본주 환산 참고값으로 구분한다. 세
활용신청 화면의 일반 인증키가 같으면 하나의 Secret만 등록한다. adapter는 인코딩/디코딩
형태를 모두 정규화한다.
...
비어 있지 않은 상품·분석 기준주식 시계열, 정확한 005930/000660 매핑, 최신값과 공통 거래일을
검증한 뒤 상품별 정적 JSON을 만든다. 하나라도 실패하면 build/upload/deploy가 실행되지 않아
직전 정상 Pages가 유지된다.
...
- 빈 분석 기준주식 시계열: 주식시세정보 활용승인과 005930/000660 응답을 확인한다. 전 상품의
  전체 분석이 완성되지 않으면 새 Pages 배포를 중단한다.
